...
if [ $? -ne 0 ]; then
  echo "Error: Could not read config.js"
  exit 1
fi
...
mysqldump -h "${HOST}" -u "${USER}" -p"${PASS}" --no-data --skip-dump-date --routines --triggers --no-tablespaces "${DB}" > schema.sql

if [ $? -eq 0 ]; then
    echo "Done. schema.sql updated."
else
