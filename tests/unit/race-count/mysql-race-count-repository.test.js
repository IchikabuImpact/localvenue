...
test('MySqlRaceCountRepositoryはcalendarから会場を読みrace_cntとrace_count_by_dateへupsertする', async () => {
  ...
  const repo = new MySqlRaceCountRepository({ pool: mockPool });
  await repo.connect();
  const codes = await repo.findVenueCodesByDate('2026-05-23');
  await repo.beginTransaction();
  await repo.saveRaceCount({ ymd: '20260523', venueCode: 31, totalRaces: 11 });
  await repo.commit();
  await repo.close();

  assert.deepEqual(codes, [31, 32]);
  // calls[0] = execute SELECT
  assert.match(calls[0][1], /SELECT venucode FROM calendar WHERE race_date = \? ORDER BY venucode/);
  assert.deepEqual(calls[0][2], ['2026-05-23']);
  assert.deepEqual(calls[1], ['beginTransaction']);
  assert.match(calls[2][1], /INSERT INTO race_cnt \(id, cnt\)/);
  assert.deepEqual(calls[2][2], ['2026052331', 11]);
  assert.match(calls[3][1], /INSERT INTO race_count_by_date \(ymd, venue_code, total_races\)/);
  assert.deepEqual(calls[3][2], ['20260523', '31', 11]);
  assert.deepEqual(calls[4], ['commit']);
  assert.deepEqual(calls[5], ['release']);
});
