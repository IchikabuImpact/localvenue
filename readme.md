...
2. **デイリー結果・予想集計バッチ**  
   指定日のレース結果取得 → 予想評価 → ROI集計 → Webページ生成までを一括実行する。
3. **デイリーWebページ生成スクリプト**  
   指定日のデータからWebページを生成する（単体実行も可能）。
...
### 2. 結果収集・評価・ページ生成バッチ (Daily Result Collection)
日次一括処理（結果収集 + 予想評価 + ROI集計 + 静的ページ生成）は以下を使用します。
```bash
# 本日分
...
```
このバッチは内部で次を順番に呼び出します。
1. `scripts/003-list-race-ids.js`
2. `scripts/101-save-result-db.js`
3. `scripts/102-eval-prediction.js`
4. `scripts/103-eval-roi.js`
5. `scripts/104-aggregate-roi-daily.js`
6. `scripts/generate-daily-pages.js`
...
指定日の静的ページを生成します。通常は `daily-result-batch.js` から自動で呼ばれますが、単体実行も可能です。
```bash
# 指定日のページを生成
node scripts/generate-daily-pages.js 20260201
```
生成結果は、最新ページを `public/`、日次アーカイブを `public/daily/YYYYMMDD/` に出力し、30日より古い日次ディレクトリは自動削除されます。
...
| スクリプト名 | 入力 (引数) | 出力 (DB) | 役割 |
| :--- | :--- | :--- | :--- |
| **`scripts/101-save-result-db.js`** | `YYYYMMDDRRBB` | `race_results`<br>`race_payouts` | 楽天競馬から結果と払戻を取得・保存します。 |
| `scripts/102-eval-prediction.js` | `YYYYMMDDRRBB` | `prediction_eval` | 予想と結果を突き合わせ、的中有無を判定して保存します。 |
| `scripts/103-eval-roi.js` | `--from` `--to` 等 | (標準出力)<br>`prediction_roi` | 期間を指定して回収率 (ROI) をシミュレーション・集計します。<br>例: `node scripts/103-eval-roi.js --from 2025-10-01 --to 2025-10-31` |
...
graph TD
    ...
    subgraph ResultFlow [Result Collection]
        E[scripts/101-save-result-db] -->|Race Results/Payouts| DB
    end

    subgraph AnalysisFlow [Analysis]
        D & E --> F[scripts/102-eval-prediction]
        F -->|Hit/Miss| DB
        DB --> G[scripts/103-eval-roi]
        G -->|ROI Stats| Console
    end
```
...
以下が README に必要な最低情報です（Node/MySQL/Chrome + DBユーザー作成まで）。

### 1) 必要ミドルウェア
- Node.js 22.x
- npm
- MySQL 8.x
- Google Chrome（Selenium利用時）

### 2) Ubuntu (WSL) での初期セットアップ例
```bash
sudo apt update
...
```sql
CREATE DATABASE localkeiba CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
CREATE USER 'localvenue'@'localhost' IDENTIFIED BY 'change_me_strong_password';
GRANT ALL PRIVILEGES ON localkeiba.* TO 'localvenue'@'localhost';
FLUSH PRIVILEGES;
```
...
```bash
cp config/config.sample.js config/config.js
# config/config.js の DB 接続情報を上記ユーザーに合わせる
npm install
node data/data_reset.js
```
...
# 静的HTML生成
node scripts/generate-daily-pages.js 20251013
...
## 🚀 VPS (Rocky Linux) デプロイ手順（静的HTML配信）

方針: **バッチはWSL/実行サーバーで実行し、VPSは静的ファイル配信のみ**。
...
### 2) 配信ディレクトリ
```bash
sudo mkdir -p /var/www/localvenue
sudo chown -R $USER:$USER /var/www/localvenue
```
...
### 4) 生成物をVPSへ転送
ローカル（WSL）で日次バッチ実行後に `public/` を転送します。
```bash
...
運用上の次ステップは `docs/spec.md` を参照してください。
