...
```
外部サイト → スクレイピング → MySQL → 予想生成 → 静的HTML → git push → VPS公開
```
...
### public/ を git 管理する理由
VPS のメモリ節約のため、静的 HTML をリポジトリに含めて `git pull` だけで公開できる構成。
バッチ機（自宅 AK1PLUS）が `git push`、VPS が `git pull`。VPS 側は nginx のみ。
